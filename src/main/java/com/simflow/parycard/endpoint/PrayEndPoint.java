package com.simflow.parycard.endpoint;

public class PrayEndPoint {

    //pray
    public static final String PRAY_ROOT = "/pray/v1";
    public static final String PRAY_CREATE = PRAY_ROOT + "/create";
    public static final String PRAY_UPDATE = PRAY_ROOT + "/update";
    public static final String PRAY_DELETE = PRAY_ROOT + "/delete";
    public static final String PRAY_LIST = PRAY_ROOT + "/list";

}
